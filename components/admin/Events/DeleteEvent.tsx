  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BACK_URL}/event/${event.id}`,
        {
          method: "DELETE",
        }
      );
      ...
      setSuccess("Evénement supprimé avec succès");
      setIsOpen(false);
      ...
      <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center">
        <div className="bg-white rounded-lg p-6 w-full max-w-md shadow-lg relative">
          <h2 className="text-lg font-semibold font-one text-secondary-500 mb-4">
            {`Confirmer la suppression : ${event.title}`}
          </h2>

          <p className="text-sm text-noir-500 mb-4">
            Es-tu sûr de vouloir supprimer cet événement ? Cette action est
            irréversible.
          </p>
